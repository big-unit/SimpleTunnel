...
class UDPServerConnection: Connection {
    ...
    func getEndpointFromSocketAddress(_ socketAddressPointer: UnsafePointer<sockaddr>) -> (host: String, port: Int)? {
		...
		switch Int32(socketAddress.sa_family) {
			case AF_INET:
                var socketAddressInet = socketAddressPointer.withMemoryRebound(to: sockaddr_in.self, capacity: 1, {$0.pointee})
			...
			case AF_INET6:
				var socketAddressInet6 = socketAddressPointer.withMemoryRebound(to: sockaddr_in6.self, capacity: 1, {$0.pointee})
    ...
    func createSocketWithAddressFamilyFromAddress(_ address: String) -> Bool {
        ...
        newResponseSource.setEventHandler {
            ...
            let bytesRead = withUnsafeMutablePointer(to: &socketAddress) {
                recvfrom(UDPSocket, UnsafeMutableRawPointer(mutating: response), response.count, 0, $0.withMemoryRebound(to: sockaddr.self, capacity: 1, {$0}), &socketAddressLength)
			}
            ...
            guard let endpoint = withUnsafePointer(to: &socketAddress, { self.getEndpointFromSocketAddress($0.withMemoryRebound(to: sockaddr.self, capacity: 1, {$0})) }) else {
		...
		switch addressFamily {
			case AF_INET:
                ...
                sent = withUnsafePointer(to: &serverAddress.sin) {
                    sendto(UDPSocket, [UInt8](data), data.count, 0, $0.withMemoryRebound(to: sockaddr.self, capacity: 1, {$0}), socklen_t(serverAddress.sin.sin_len))
				}
			...
			case AF_INET6:
                ...
                sent = withUnsafePointer(to: &serverAddress.sin6) {
                    sendto(UDPSocket, [UInt8](data), data.count, 0, $0.withMemoryRebound(to: sockaddr.self, capacity: 1, {$0}), socklen_t(serverAddress.sin6.sin6_len))
				}
