...
class ServerTunnelConnection: Connection {
	...
	func sendOpenResult(_ result: TunnelConnectionOpenResult, extraProperties: [String: Any] = [:]) {
		guard let serverTunnel = tunnel else { return }

		var resultProperties = extraProperties
		resultProperties[TunnelMessageKey.ResultCode.rawValue] = result
	...
	func createTUNInterface() -> Int32 {
        ...
        let connectResult = connect(utunSocket, withUnsafePointer(to: &socketAddressControl, {$0.withMemoryRebound(to: sockaddr.self, capacity: 1, {$0})}), socklen_t(MemoryLayout<sockaddr_ctl>.size))

        if let errorString = String(cString: strerror(errno), encoding: .utf8), connectResult < 0 {
	...
	func getTUNInterfaceName(_ utunSocket: Int32) -> String? {
        ...
        if let errorString = String(cString: strerror(errno), encoding: .utf8), resultCode < 0 {
	...
	func readPackets() {
        ...
        var iovecList = [ iovec(iov_base: &protocolNumber, iov_len: MemoryLayout<UInt32>.size), iovec(iov_base: &buffer, iov_len: buffer.count) ]
		...
		repeat {
			...
			guard readCount > 0 || errno == EAGAIN else {
                if let errorString = String(cString: strerror(errno), encoding: .utf8), readCount < 0 {
            ...
            guard readCount > MemoryLayout<UInt32>.size else { break }
			...
			packets.append(Data(bytes: &buffer, count: readCount - MemoryLayout<UInt32>.size))
	...
	override func sendPackets(_ packets: [Data], protocols: [NSNumber]) {
		...
		for (index, packet) in packets.enumerated() {
            ...
            var iovecList = [ iovec(iov_base: &protocolNumber, iov_len: MemoryLayout<UInt32>.size), iovec(iov_base: &buffer, iov_len: packet.count) ]

			let writeCount = writev(utunSocket, &iovecList, Int32(iovecList.count))
			if writeCount < 0 {
                if let errorString = String(cString: strerror(errno), encoding: .utf8) {
					simpleTunnelLog("Got an error while writing to utun: \(errorString)")
				}
			}
			else if writeCount < packet.count + MemoryLayout<UInt32>.size {
